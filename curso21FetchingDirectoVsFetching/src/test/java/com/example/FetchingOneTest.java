...
public class FetchingOneTest {
    ...
    void findAuthorById() {
        ...
        // Si es EAGER se recupera el address junto al autor desde el principio
        var a1 = session.find(Author.class, 1L);
        System.out.println("-----------------");

        //Si se cierra la sesion antes del LAZY dará error porque tiene que volver a conectarse para recuperar los datos
    ...
    void findAuthorByIdWithAddress() {
        ...
        var session = HibernateUtil.getSessionFactory().openSession();
        //join fetch provoca que, aunque en la asociacion de la entity hayamos puesto LAZY, se traiga la direccion
        String jpql = "select a from Author a join fetch a.address where a.id = :id";

...
public class EntityGraphTest {
    ...
    void entityGraph_1level(){
        ...
        //En esta consulta nos traemos solo los libros asociados al autor
    ...
    void entityGraph_2level(){
        createData();
        var session = HibernateUtil.getSessionFactory().openSession();
        //En esta consulta traemos libros, y los capitulos asociados a los libros
        EntityGraph<Author> graph = session.createEntityGraph(Author.class);
        graph.addAttributeNodes("books");
        ...
        session.createQuery("select a from Author a", Author.class)
                ...
                .forEach(book -> System.out.println(book.getChapters().size()))
                );
    ...
    void entityGraph_2level_Graphexpression(){
        ...
        //Misma consulta que el test de arriba pero simplificada
        EntityGraph<Author> graph = GraphParser.parse(Author.class, "books(chapters)", session);
    ...
    @Test
    /*En este ejemplo llamamos a la EntityGraoh que se ha creado en la clase Author, y hara lo que alli le hallamos
    * indicado*/
